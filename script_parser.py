...
parser.add_argument('--dir', dest='path', action='store', help='Path to logfile')
...
def parse_logs(path: str):
    ...
    for i in requests_by_time_sorted:
        top_requests_by_time.append(i[0])

    result = {
        "TOTAL REQUESTS NUMBER": request_count,
        "REQUESTS NUMBER BY HTTP METHOD": requests_by_method,
        "3 TOP IPS": sorted(requests_by_ip.items(), key=lambda kv: kv[1], reverse=True)[:3],
        "3 TOP REQUESTS": top_requests_by_time
    }
